...
const WECOM_WEBHOOK_URL_ZH =
  process.env.WECOM_WEBHOOK_URL_ZH || process.env.WECOM_WEBHOOK_URL;
const WECOM_WEBHOOK_URL_EN =
  process.env.WECOM_WEBHOOK_URL_EN || process.env.WECOM_WEBHOOK_URL;
const TELEGRAM_BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN;
const TELEGRAM_CHAT_ID_ZH =
  process.env.TELEGRAM_CHAT_ID_ZH || process.env.TELEGRAM_CHAT_ID;
const TELEGRAM_CHAT_ID_EN =
  process.env.TELEGRAM_CHAT_ID_EN || process.env.TELEGRAM_CHAT_ID;
...
async function loadTextGenerator() {
  const { generateTextContent } = await import(
    "../src/pages/daily/textGenerator.ts"
  );
...
async function takeScreenshots(languages = ["zh"], dateStr = null) {
  ...
  for (const language of languages) {
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      console.log(
        `Card ${i + 1} (${language}) screenshot saved to: ${filepath}`
      );
    }
...
async function sendWecomNotification(imagePath, textContent, language = "zh") {
  const webhookUrl =
    language === "zh" ? WECOM_WEBHOOK_URL_ZH : WECOM_WEBHOOK_URL_EN;

  if (!webhookUrl) {
    throw new Error(
      `WECOM_WEBHOOK_URL${language.toUpperCase()} environment variable is not set`
    );
  }
...
async function sendTelegramNotification(
  ...
  const chatIdConfig =
    language === "zh" ? TELEGRAM_CHAT_ID_ZH : TELEGRAM_CHAT_ID_EN;
...
program
  ...
  .action(async (options) => {
    try {
      const languages =
        options.language === "all" ? ["zh", "en"] : [options.language];
...
program
  ...
  .action(async (options) => {
    try {
      ...
      if (!todayData) {
        console.error(
          `No data found for ${dateString}. Aborting notification.`
        );
        process.exit(1);
      }
      ...
      for (const lang of languages) {
        ...
        if (
          TELEGRAM_BOT_TOKEN &&
          (TELEGRAM_CHAT_ID_ZH || TELEGRAM_CHAT_ID_EN)
        ) {
      ...
      if (!fs.existsSync(dataFilePath)) {
        ...
        if (existingEntry) {
          ...
          if (convertedCount > 0 || qasInitialized) {
            ...
            console.log(
              `Entry for ${dateString} already exists, no conversion needed`
            );
          }
          return;
        }
