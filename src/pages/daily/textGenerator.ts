...
export const generateTextContent = (
  ...
  cards.forEach((card) => {
    ...
    if (
      language !== "zh" &&
      card.languages &&
      !card.languages.includes(language)
    ) {
        ...
        if (show_details) {
          ...
          if (project.notes) {
            ...
            if (
              ...
              Array.isArray(project.notes[language])
            ) {
    ...
    if (show_qas && card.qas && card.qas.length > 0) {
      const qaHeading =
        language === "zh" ? "💬 今日要点 深入解析" : "💬 Q&A Deep Dive";
      text += `${qaHeading}\n\n`;
      card.qas.forEach((qa: any, index: number) => {
        const question =
          qa.question[language] || qa.question.en || qa.question.zh;
